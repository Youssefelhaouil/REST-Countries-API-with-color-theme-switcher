...
const CountryInfo = ({ isDark, countries }) => {
  ...
  const country = countries.find(
    (country) => country.area === Number(area)
  );
  ...
  return (
    <>
      <div className="px-[100px] py-[20px] md:px-4 md:py-4  ">
        <Link
          className={`px-3 py-2 rounded-lg h-8 w-[100px] flex gap-2 items-center font-semibold font-sans text-lg ${
            isDark
              ? "bg-dark-blue hover:bg-very-dark-blue text-white"
              : "bg-gray-200 hover:bg-gray-100 text-very-dark-blue"
          } `}
          to={"/"}
        >
          {" "}
        ...
        <div
          className={`pt-8 flex md:flex-col gap-[40px] rounded-xl ${
            isDark ? "text-white " : "text-very-dark-blue "
          }`}
        >
          <img
            src={country.flags?.png}
            alt={`Flag of ${country.name.common}`}
            className="w-full rounded-xl h-[450px] md:h-[300px] max-w-lg mb-6"
          />
